...
pair_file = "pairs/pairs-2022-11-03.csv"
...
def filt(name):
    name = str(name)
    name = name.replace("mw brain and inputs", "brain_and_inputs")
    name = name.replace("mw brain neurons", "brain_neurons")
...
print("All neurons:", len(all_neurons))

# TODO ask michael about this part
# brain_and_inputs = get_indicator_from_annotation("mw brain and inputs").index
# accessory = get_indicator_from_annotation("mw brain accessory neurons").index
# all_neurons = all_neurons.union(brain_and_inputs).union(accessory)
print("All neurons with brain and inputs and accessory:", len(all_neurons))

meta = pd.DataFrame(index=all_neurons)
print(f"\n{len(meta)} neurons in meta.\n")
...
meta.drop(columns=neuron_group_cols, inplace=True)
...
for i in range(1, n_batches):
    ...
    while not success and n_tries < max_tries:
        try:
            ...
            success = True
        except ChunkedEncodingError or ConnectionError:

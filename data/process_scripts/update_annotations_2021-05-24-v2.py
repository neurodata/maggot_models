...
old_path = Path(f"maggot_models/data/processed/{old_name}")
shutil.copytree(old_path, output_path)
...
pair_file = "pairs/pairs-2021-04-06.csv"
...
#%%
old_meta = pd.read_csv(output_path / "meta_data.csv", index_col=0)
print(old_meta.index.difference(meta.index))
print(meta.index.difference(old_meta.index))
meta.rename(index={21790197: 15672263}, inplace=True)
print(meta.index.difference(old_meta.index))
meta = meta.reindex(index=old_meta.index)
print((old_meta['simple_group'] != meta['simple_group']).sum())
old_meta['simple_group'] = meta['simple_group']
print((old_meta['simple_group'] != meta['simple_group']).sum())
old_meta.to_csv(output_path / 'meta_data.csv')
# old_meta['simple_group'] = meta['simple_group']

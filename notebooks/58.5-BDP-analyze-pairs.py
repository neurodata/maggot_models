...
edge_max_weight_map = dict(zip(max_pair_edge_df.index.values, max_pair_edge_df["weight"]))
edgelist_df["max_weight"] = itemgetter(*edgelist_df["edge pair ID"])(
    edge_max_weight_map
)
# %% [markdown] 
# # 
path = "maggot_models/notebooks/outs/58.5-BDP-analyze-pairs"
asym_pair_edge_df = max_pair_edge_df[max_pair_edge_df["edge pair counts"] == 1].copy()
asym_pair_edge_df.sort_values("weight", ascending=False, inplace=True)
asym_pair_edge_df.to_csv(path + "/asym_edges.csv")

# %% [markdown] 
# # Do the above but per neuron 
thresh_df = edgelist_df[edgelist_df["max_weight"] > 0.01]
asym_pair_edge_df = thresh_df[thresh_df["edge pair counts"] == 1].copy()
source_asym_size = asym_pair_edge_df.groupby("source").size()
target_asym_size = asym_pair_edge_df.groupby("target").size()
ids = np.union1d(source_asym_size.index, target_asym_size.index).astype('int64')
n_asym_df = meta.loc[ids]
n_asym_df["n_asym_edges"] = -1
asym_df = pd.DataFrame({"source":source_asym_size, "target":target_asym_size})
asym_df.index = asym_df.index.astype('int64')
...
#%% do the above but by neuron
# probably a way better way to do this... 
source_ids = np.unique(edgelist_df["source"].astype('int64'))
target_ids = np.unique(edgelist_df["target"].astype('int64'))
ids = np.union1d(source_ids, target_ids)
n_asym_df = meta.loc[ids]
n_asym_df["n_asym_edges"] = -1
n_asym_df["n_edges"] = -1
n_asym_df["p_asym_edges"] = -1
for pid in ids:
    temp_df = edgelist_df[
        (edgelist_df["source"] == pid) | (edgelist_df["target"] == pid)
    ]
    n_edges = len(temp_df)
    temp_df = temp_df[temp_df["edge pair counts"] == 1]
    n_asym_edges = len(temp_df)
    n_asym_df.loc[pid, "n_asym_edges"] = n_asym_edges
    n_asym_df.loc[pid, "n_edges"] = n_edges
    if n_edges > 0:    
        n_asym_df.loc[pid, "p_asym_edges"] = n_asym_edges / n_edges

...
class PartitionCluster:
    ...
    def predict(self, X, y=None):
        if self.left_ is None:
            ...
            left_counter = 0
            right_counter = 0
            print(full_pred_labels)
            for i, went_left in enumerate(indicator):
                print("here")
                if went_left:
                    x = left_pred[left_counter].append(0)
                    full_pred_labels.append(x)
                    left_counter += 1
                    print(left_counter)
                else:
                    print(right_pred[right_counter])
                    x = right_pred[right_counter].append(1)
                    full_pred_labels.append(x)
                    right_counter += 1
            print(full_pred_labels)
...
X1, y1 = make_blobs(n_samples=100, n_features=3, centers=None, cluster_std=3)
X1_df = pd.DataFrame(
    data=np.concatenate((X1, y1[:, np.newaxis]), axis=-1),
    columns=("Dim 0", "Dim 1", "Dim 2", "Labels"),
)

X2, y2 = make_blobs(n_samples=100, n_features=3, centers=None, cluster_std=2)
X2 += np.array((40, 40, 40))[np.newaxis, :]
y2 += 3
X2_df = pd.DataFrame(
    data=np.concatenate((X2, y2[:, np.newaxis]), axis=-1),
    columns=("Dim 0", "Dim 1", "Dim 2", "Labels"),
)

X_df = pd.concat((X1_df, X2_df))

sns.scatterplot(
    data=X_df, x="Dim 1", y="Dim 2", hue="Labels", palette="Set1", legend=False
)
X = X_df.values[:, :3]

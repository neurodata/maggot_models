...
# means = umap_embedding.mean(axis=0)
umap_embedding -= mids  # means[None, :]
max_length = np.linalg.norm(umap_embedding, axis=1).max()
umap_embedding /= max_length
# maxs = np.abs(umap_embedding)
# print(umap_embedding.mean(axis=0))
# extents = np.umap_embedding.max(axis=0) - umap_embedding.min(axis=0)
# max_extent = extents.max()
# umap_embedding /= max_extent
...
# ax.axvline(0)
# ax.axhline(0)
# norms = np.linalg.norm(umap_embedding, ord=2, axis=1)
# max_norm = norms.max()
...
plot_neuron_ids = np.random.choice(meta.index, replace=False, size=n_plot_neurons)
...
ax.relim()
ax.autoscale_view()
stashfig(f"temp-maggot-brain-umap-omni-hue_key={hue_key}")

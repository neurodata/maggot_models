...
for i in range(n_tsne):
    ...
    print(kl)
    if kl < best_kl:
        best_embed = cos_embed
        best_kl = kl
cos_embed = best_embed
# %% [markdown]
# ##
...
# %% [markdown]
# ## Look at path distance mat

basename = f"-subsample={subsample}-plen={path_len}-graph={graph_type}-reverse"
...
# %% [markdown]
# ## plot by cluster
pal = sns.color_palette("husl", n_colors=path_len - 1)
# pal = [pal[0], pal[2], pal[4], pal[6], pal[1], pal[3], pal[5], pal[7], (0, 0, 0)]
pal = pal[:path_len]
label = 10
mask = pred == label
...
ax.set_title("Link")
ax.set_yticklabels(np.arange(1, path_len + 1))
stashfig(f"cluster-{label}-tsne" + basename)

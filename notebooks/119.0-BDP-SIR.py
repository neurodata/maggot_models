...
class SIRCascades:
    def __init__(self, transition_probs, n_samples=100):
        ...
        self.stop_nodes = []
        self.max_hops = 10
        self.n_verts = transition_probs.shape[0]
        pass

    def start(self, start_nodes):
        pmf = np.zeros(self.n_verts)
        pmf[start_nodes] = 1 / len(start_nodes)
        n_active = len(start_nodes)  # ceil(len(start_nodes) * 0.66)
        pre_hop_hist = np.zeros((self.max_hops, self.n_verts))
        post_hop_hist = np.zeros((self.max_hops, self.n_verts))
        csc = Cascade(
        ...
        for hop in range(self.max_hops):
            next_pmf = np.zeros(self.n_verts)
            for i in range(self.n_samples):
                start_nodes = np.random.choice(
                    self.n_verts, size=n_active, replace=False, p=pmf
                )
                pre_hop_hist[hop, start_nodes] += 1
                csc.start(start_nodes)
                active = csc._active
                next_pmf[active] += 1
                post_hop_hist[hop, active] += 1
            next_pmf /= next_pmf.sum()
            pmf = next_pmf
        return pre_hop_hist, post_hop_hist


p = 0.03
transition_probs = to_transmission_matrix(adj, p)
sirc = SIRCascades(transition_probs, n_samples=1000)
pre, post = sirc.start(np.arange(n_feedforward))

fig, ax = plt.subplots(1, 1, figsize=(10, 5))
matrixplot(pre, ax=ax, col_sort_class=labels)

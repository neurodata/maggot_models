...
for quantile in quantiles:
    ...
    ase = AdjacencySpectralEmbed(
        n_components=None, n_elbows=2, check_lcc=False, n_iter=10
    )
    embed = ase.fit_transform(pass_to_ranks(adj))
    embed = np.concatenate(embed, axis=1)

    pc = Procrustes()
    pc.fit(embed, x_seeds=lp_inds, y_seeds=rp_inds)
    embed = pc.transform(embed, map_inds=left_inds)
...
# conclusion: it appears the procedure I was using for spectral/alignment may be
# pretty crappy for low degree stuff. Note how pairs aren't that close and the embedding
# just looks "smeared"

...
left_inds = np.where(mg.meta["Hemisphere"] == "L")[0]
right_inds = np.where(mg.meta["Hemisphere"] == "R")[0]
to_class_inds = np.where(mg.meta["Class 1"].isin(to_classes).values)[0]
...
# left_edges = np.array([0, 2, 2, 5, 0, 1])
# right_edges = np.array([0, 0, 1, 5, 1, 2])


props = []
# threshs = [4]  #
threshs = np.linspace(0, 0.1, 200)

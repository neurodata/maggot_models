...
from graspy.utils import augment_diagonal, binarize, pass_to_ranks, symmetrize
...
def lateral_omni(adj, lp_inds, rp_inds, n_components=4):
    ...
    ipsi_embed = omni([ll_adj, rr_adj], n_components=n_components)

    lr_adj = adj[np.ix_(lp_inds, rp_inds)]
    rl_adj = adj[np.ix_(rp_inds, lp_inds)]

    # TODO here should be giving some of these to left node, some to right node
    contra_embed = omni([lr_adj, rl_adj], n_components=n_components)
...
def multi_lateral_omni(adjs, lp_inds, rp_inds, n_components=4):
    ...
    ipsi_embed = omni(ipsi_adjs, concatenate=False, n_components=n_components)
    left = []
    right = []
    ...
    contra_embed = omni(contra_adjs, concatenate=False, n_components=n_components)
    left = []
    right = []
...
n_omni_components = 4  # this is used for all of the embedings initially
...
omni_flat_embed = lateral_omni(adj, lp_inds, rp_inds, n_components=n_omni_components)
svd_flat_embed = svd(omni_flat_embed)
...
svd_multi_embed = svd(omni_multi_embed)
...
svd_reg_embed = svd(omni_reg_embed)
...
svd_joint_embed = svd(omni_joint_embed)
...
names = ["flat", "multi", "joint", "reg"]
embeds = [svd_flat_embed, svd_multi_embed, svd_joint_embed, svd_reg_embed]
dims = np.arange(1, 16)
dfs = []
for d in dims:
...
stashfig("svd-reg-pairs")

quick_embed_viewer(
    svd_reg_embed[:, :8], labels=labels, lp_inds=new_lp_inds, rp_inds=new_rp_inds
)
stashfig("svd-reg-manifold")
...
def class_neighbors_at_k(X, labels, k_max=10, metric="euclidean"):
    nn = NearestNeighbors(radius=0, n_neighbors=k_max + 1, metric=metric)
    nn.fit(X)
    neigh_dist, neigh_inds = nn.kneighbors(X)
...
axs[0].set_ylabel(f"P @ K = {k}")
axs[3].set_ylabel(f"P @ K = {k}")
...
d = 8
flat_embed = svd_flat_embed[:, :d]
color_embed = svd_reg_embed[:, :d]


def pair_diff(X, lp_inds, rp_inds):
    return np.linalg.norm(X[lp_inds] - X[rp_inds], axis=1)
...
pair_meta = new_meta.copy()
pair_meta = pair_meta[keep_cols]
pair_meta.index.name = "skid"
pair_meta = pair_meta.reset_index()
left_meta = pair_meta.iloc[new_lp_inds].copy()
right_meta = pair_meta.iloc[new_rp_inds].copy()
right_meta.index = left_meta.index
assert (left_meta["Pair ID"].values == right_meta["Pair ID"].values).all()
left_meta.rename(columns=lambda x: x + "_left", inplace=True)
right_meta.rename(columns=lambda x: x + "_right", inplace=True)
...
for metric in ["euclidean", "cosine"]:
    ...
    flat_diff = paired_distances(
        flat_embed[new_lp_inds], flat_embed[new_rp_inds], metric=metric
    )
    color_diff = paired_distances(
        color_embed[new_lp_inds], color_embed[new_rp_inds], metric=metric
    )
    # rank (putting big differences first)
    flat_diff_ranks = rankdata(-flat_diff)
    color_diff_ranks = rankdata(-color_diff)

    pair_meta[f"flat_diff_{metric}"] = flat_diff
    pair_meta[f"color_diff_{metric}"] = color_diff
    pair_meta[f"rank_flat_{metric}"] = flat_diff_ranks
    pair_meta[f"rank_color_{metric}"] = color_diff_ranks

pair_meta.sort_values(f"rank_flat_euclidean", inplace=True)
pair_meta.head(20)
# stashcsv(pair_meta, "pair_distances")

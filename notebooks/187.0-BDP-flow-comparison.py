...
rows = []
for i, g1 in enumerate(graph_types):
    source_mg = mg_dict[g1]
    source_adj = source_mg.adj
    source_meta = source_mg.meta
    sf = source_meta[f"{g1}_flow"]
    perm_inds = np.argsort(sf)
    perm_source_adj = source_adj[np.ix_(perm_inds, perm_inds)]
    prop = calc_upper_triu(perm_source_adj, method=method)
    for j, g2 in enumerate(graph_types):
        ax = axs[i, j]

        target_mg = mg_dict[g2]
        target_adj = target_mg.adj
        target_meta = target_mg.meta
        perm_target_adj = target_adj[np.ix_(perm_inds, perm_inds)]
        perm_target_meta = target_meta.iloc[perm_inds]
        _, _, top, _, = adjplot(
            perm_target_adj,
            meta=perm_target_meta,
            colors="merge_class",
            palette=CLASS_COLOR_DICT,
            plot_type="scattermap",
            sizes=(0.5, 1),
            ax=ax,
            color=graph_type_colors[g2],
        )
        if i == 0:
            ax.set_title(graph_names[g2], color=graph_type_colors[g2], pad=10)
        if j == 0:
            ax.set_ylabel(graph_names[g1], color=graph_type_colors[g1], labelpad=10)
...
fig.text(0.515, 0.905, "Target graph", ha="center", fontsize="x-large")

# row = {"p_upper_triu": prop, "type": "True", "method": "sum", "graph_type": g}
# rows.append(row)
stashfig("adjplot-pairwise-sorting")


# %%

line_kws = dict(linewidth=1, linestyle="--", color="black")


def plot_diag_vals(adj, graph_type, ax):
    ks = np.arange(-len(adj) + 1, len(adj))
...
for i, g1 in enumerate(graph_types):
    ...
    for j, g2 in enumerate(graph_types):
        ...
        target_meta = target_mg.meta
        perm_target_adj = target_adj[np.ix_(perm_inds, perm_inds)]
        plot_diag_vals(perm_target_adj, g2, ax)
        if i == 0:
            ax.set_title(graph_names[g2], color=graph_type_colors[g2], pad=10)
        if j == 0:
            ax.set_ylabel(graph_names[g1], color=graph_type_colors[g1], labelpad=10)
...
# row = {"p_upper_triu": prop, "type": "True", "method": "sum", "graph_type": g}
# rows.append(row)
stashfig("diagplot-pairwise-sorting")
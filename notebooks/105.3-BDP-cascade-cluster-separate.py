...
for fg, fg_name in zip(from_groups, from_group_names):
    ...
    stashcsv(pd.DataFrame(data=hop_hist), f"{fg_name}-hop-hist" + basename)
...
for i, (fg, fg_name) in enumerate(zip(from_groups, from_group_names)):
    ...
    ggmm = GaussianCluster(
        min_components=10, max_components=40, n_init=20, covariance_type="diag"
    )
    ggmm.fit(X)
    ...
    bics = gbics.copy()
    bics.index.rename("K", inplace=True)
    bics = bics.reset_index()
    bics = bics.melt(id_vars="K")
    best_graspy = bics.groupby("K").min()
    best_graspy = best_graspy.reset_index()
    sns.scatterplot(
        data=best_graspy, y="value", x="K", label="GraspyGMM", ax=ax, **scatter_kws
    )
    ax.set_ylabel("BIC")
    ax.set_xlabel("K")

    stashfig(f"{fg_name}-bic" + basename)


# %% [markdown]
# ##
ks = [30, 20, 23, 29, 25, 32]
autogmm = False
for i, (fg, fg_name) in enumerate(zip(from_groups, from_group_names)):
    print(f"Plotting best for {fg_name}")
    ...
    if autogmm:
        ...
        # for graspy
        gmm = GaussianCluster(
            min_components=k, max_components=k, covariance_type="diag", n_init=40
        )
        gmm.fit(X)
        model = gmm.model_

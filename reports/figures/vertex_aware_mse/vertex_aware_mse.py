...
def get_best(df, param_name="param_n_components", score_name="mse"):
    ...
    for param_value in param_range:
        ...
        ind = temp_df[score_name].idxmax()  # this is the metric we are choosing on
        best_rows.append(temp_df.loc[ind, :])
    return pd.DataFrame(best_rows)


best_dcsbm_df = get_best(dcsbm_df, param_name="param_n_blocks", score_name="likelihood")
best_ddcsbm_df = get_best(
    ddcsbm_df, param_name="param_n_blocks", score_name="likelihood"
)
best_rdpg_df = get_best(
    rdpg_df, param_name="param_n_components", score_name="likelihood"
)
